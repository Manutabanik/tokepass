...
export function CheckoutHeader({
  ...
  return (
    <header className="sticky top-0 z-40 w-full flex-none border-b border-border/50 bg-background/70 backdrop-blur-xl">
      <div className="flex h-14 w-full items-center justify-between gap-2 px-4 md:hidden">
        <button
          type="button"
          onClick={onBack}
          className="flex min-h-11 items-center gap-1.5 rounded-lg bg-secondary px-2.5 py-1.5 text-xs font-bold text-secondary-foreground transition-all active:scale-95"
        >
          <ArrowLeft className="size-4 text-primary" aria-hidden="true" />
          <span>Volver</span>
        </button>

        <div className="flex items-center gap-1.5 rounded-full border border-border/40 bg-secondary px-3 py-1">
        ...
        {timer ?? (
          <span
            className={cn(
              "min-w-12 text-right text-[11px] font-semibold text-muted-foreground",
              !showLimit && "invisible",
            )}
          >
