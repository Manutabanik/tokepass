-- P152: Event Creator V2 publish unpack.
-- One transaction: venue upsert, general/addon ticket merge, event fields,
-- then clear draft_state and mark the event published.
...
begin
    ...
    if v_tier_id is not null then
      ...
      if found then
        ...
        set
          ...
          tier_type = case
            when v_tier ->> 'tier_type' in ('general', 'addon') then v_tier ->> 'tier_type'
            else 'general'
          end,
          category = case
            when v_tier ->> 'category' in ('standard', 'special') then v_tier ->> 'category'
            else 'standard'
          end,
          layout_type = 'general',
  ...
  if exists (
    ...
    where tt.event_id = p_event_id
      and tt.tier_type in ('general', 'addon')
...
comment on function public.publish_event_v2(uuid, jsonb) is
  'Event Creator V2: unpacks validated draft JSON into events, venues and general/addon ticket_tiers, then clears draft_state.';
