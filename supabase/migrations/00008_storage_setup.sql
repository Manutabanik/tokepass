-- =============================================================================
-- Tokepass - Supabase Storage para flyers de eventos
-- Nota: 00007 ya existe (Mercado Pago). Este archivo es 00008.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- Columna flyer_url (image_url se mantiene por compatibilidad)
-- -----------------------------------------------------------------------------

alter table public.events
  add column if not exists flyer_url text;

comment on column public.events.flyer_url is
  'URL pública del flyer en el bucket event-flyers';

-- -----------------------------------------------------------------------------
-- Bucket público event-flyers
-- -----------------------------------------------------------------------------
...
drop policy if exists "event_flyers_public_select" on storage.objects;
...
create policy "event_flyers_public_select"
on storage.objects
for select
to public
using (bucket_id = 'event-flyers');

-- Escritura solo organizadores / super admins
...
-- -----------------------------------------------------------------------------
-- RPC: aceptar flyer_url y persistir en flyer_url + image_url
-- -----------------------------------------------------------------------------
...
begin
  ...
  -- Preferimos flyer_url (Storage); image_url queda como alias legacy.
  ...
  if payload ? 'rrpp_commission'
    ...
    exception
      when others then
        raise exception 'rrpp_commission inválida' using errcode = '22023';
    end;
    ...
    insert into public.promoters (
      event_id, profile_id, commission_percentage, custom_link
    )
    values (v_event_id, p_organizer_id, v_rrpp_commission, v_custom_link);
  end if;
...
comment on function public.create_complete_event_tx(jsonb, uuid) is
  'Crea de forma atómica venue, event (draft + flyer), zones, seats, tiers y promoter RRPP.';
