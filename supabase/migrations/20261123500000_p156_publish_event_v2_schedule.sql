-- P156: unpack multi-day schedule from draft JSON.
-- Writes events.date / ends_at from the first/last day and events.schedule_days
-- so the existing JSONB <-> event_schedules trigger replaces jornadas atomically.
-- Keeps P155: do not assign ticket_tiers.category from jsonb text.
...
comment on function public.publish_event_v2(uuid, jsonb) is
  'Event Creator V2: unpacks draft JSON into events/venues/ticket_tiers/schedule_days. Leaves ticket_tiers.category on its ENUM default.';
