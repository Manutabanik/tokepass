-- =============================================================================
-- P0 hardening: pending_payment tickets, refund+stock RPC, webhook ledger
-- composite key, featured column locks.
-- =============================================================================

-- Default new bare inserts to pending_payment (POS/freepass set valid explicitly).
alter table public.tickets
  alter column status set default 'pending_payment'::public.ticket_status;

-- Backfill unpaid reservations still marked valid.
update public.tickets as t
set
  status = 'pending_payment'::public.ticket_status,
  updated_at = now()
...
-- -----------------------------------------------------------------------------
-- release_reserved_tickets: only pending_payment (never paid `valid`)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- reserve_tickets_tx: insert pending_payment; anti-scalp counts holds + valid
-- -----------------------------------------------------------------------------
...
comment on function public.reserve_tickets_tx(uuid, uuid, jsonb, uuid) is
  'Reserva tickets en pending_payment hasta webhook approved; anti-scalp + service charge.';
...
-- -----------------------------------------------------------------------------
-- Atomic refund / chargeback: cancel valid tickets + restore tier sold
-- -----------------------------------------------------------------------------
...
revoke all on function public.cancel_paid_order_tickets(uuid) from anon, authenticated;
grant execute on function public.cancel_paid_order_tickets(uuid) to service_role;

-- Activate pending_payment → valid when order paid (service_role / webhook).
create or replace function public.activate_order_tickets(p_order_id uuid)
returns integer
language plpgsql
security definer
set search_path = ''
...
revoke all on function public.activate_order_tickets(uuid) from public;
revoke all on function public.activate_order_tickets(uuid) from anon, authenticated;
grant execute on function public.activate_order_tickets(uuid) to service_role;

-- Scanner / wallet eligibility (bypasses orders RLS for organizers).
...
-- -----------------------------------------------------------------------------
-- Webhook ledger: idempotency by (payment_id, status)
-- -----------------------------------------------------------------------------
alter table public.mp_webhook_events
  drop constraint if exists mp_webhook_events_pkey;

alter table public.mp_webhook_events
  add primary key (payment_id, status);

create index if not exists mp_webhook_events_payment_id_idx
  on public.mp_webhook_events (payment_id);

-- -----------------------------------------------------------------------------
-- Featured / Boost columns: clients cannot UPDATE; service_role can
-- -----------------------------------------------------------------------------
revoke update (is_featured, featured_tier, featured_until)
  on public.events from authenticated, anon;

grant update (is_featured, featured_tier, featured_until)
...
drop trigger if exists events_featured_columns_guard on public.events;
create trigger events_featured_columns_guard
  before update on public.events
  for each row
  execute function public.enforce_featured_columns_service_role();

-- Transfer: reject tickets without a paid order (freepass without order still ok).
...
begin
  ...
  if v_ticket.order_id is not null then
    ...
    if v_order_status is distinct from 'paid' then
      raise exception 'TICKET_NOT_PAID' using errcode = '23514';
    end if;
  end if;
