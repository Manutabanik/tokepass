-- P158: unlink ticket_tiers.day_id before syncing event_schedules.
-- The events.schedule_days trigger deletes leftover jornadas and raises 23503
-- when ticket_tiers still point at them. Children first, then upsert days,
-- then re-bind day_id.
...
comment on function public.publish_event_v2(uuid, jsonb) is
  'Event Creator V2: unpacks draft JSON into events/venues/ticket_tiers/schedule_days. Unlinks ticket_tiers.day_id before syncing event_schedules to avoid 23503.';
