-- P206 · Sold/reserved units keep their original tier_id when the map
-- recategorizes the same layout_item_id into another sector.
...
begin
  ...
  order by case when u.sector_id = p_sector_id then 0 else 1 end
  limit 1
  for update;
...
comment on function public.upsert_event_seating_unit_row(
  ...
  'Writes one unit per physical seat. Sold/reserved rows keep their original tier_id when the map moves them to another sector.';
