-- P157: do not assign events.refund_policy from JSON text.
-- V2 stores buyer-facing refund copy in description; the ENUM stays on the
-- existing row (platform / SuperAdmin). Avoids Postgres 42804 text vs enum.
...
comment on function public.publish_event_v2(uuid, jsonb) is
  'Event Creator V2: unpacks draft JSON into events/venues/ticket_tiers/schedule_days. Leaves events.refund_policy and ticket_tiers.category on their ENUM defaults.';
