-- P188: persist purchase fields inside publish_event_v2 (same TX as tickets).
-- P158 dropped ticket_type (P162). Extras published via RPC stayed "standard"
-- and checkout treated them as admission. Also writes refund_policy,
-- checkout_message, absorb_fees, access_link, category, sale windows, and
-- clears events.venue_map when the seating plan is off.
...
begin
  ...
  v_refund := case
    when p_payload ->> 'refund_policy' in ('organizer', 'no_refunds', 'until_24h')
      then p_payload ->> 'refund_policy'
    else 'organizer'
  end;
  v_access_link := nullif(btrim(p_payload ->> 'access_link'), '');
  v_checkout := nullif(btrim(p_payload ->> 'checkout_message'), '');
  begin
    v_absorb := coalesce((p_payload ->> 'absorb_fees')::boolean, false);
  exception
    when others then
      v_absorb := false;
  end;
  ...
  loop
    ...
    begin
      v_day_id := nullif(btrim(v_tier ->> 'day_id'), '')::uuid;
    exception
      when others then
        v_day_id := null;
    end;
    ...
    v_ticket_type := case
      when v_tier ->> 'ticket_type' in ('standard', 'combo', 'extra')
        then v_tier ->> 'ticket_type'
      when v_tier_type = 'addon' then 'extra'
      when v_tier_type = 'bundle' then 'combo'
      else 'standard'
    end;
    v_category := case
      when v_tier ->> 'category' in ('standard', 'special')
        then v_tier ->> 'category'
      when v_ticket_type = 'extra' then 'special'
      else 'standard'
    end;
    begin
      v_sale_starts := nullif(btrim(v_tier ->> 'sale_starts_at'), '')::timestamptz;
    exception
      when others then
        v_sale_starts := null;
    end;
    begin
      v_sale_ends := nullif(btrim(v_tier ->> 'sale_ends_at'), '')::timestamptz;
    exception
      when others then
        v_sale_ends := null;
    end;
    ...
    if v_tier_id is not null then
      ...
      if found then
        ...
        set
          ...
          category = v_category,
          sale_starts_at = v_sale_starts,
          sale_ends_at = v_sale_ends,
        ...
        v_bind_tier_ids := array_append(v_bind_tier_ids, v_tier_id);
        v_bind_day_ids := array_append(v_bind_day_ids, v_day_id);
        continue;
      end if;
  ...
  v_schedule := case
    when p_payload ? 'schedule_days'
         and jsonb_typeof(p_payload -> 'schedule_days') = 'array'
      then p_payload -> 'schedule_days'
    else '[]'::jsonb
  end;
  ...
  set
    ...
    refund_policy = v_refund::public.event_refund_policy,
    access_link = v_access_link,
    checkout_message = v_checkout,
    absorb_fees = v_absorb,
  ...
  if cardinality(v_bind_tier_ids) >= 1 then
    for v_bind_idx in 1 .. cardinality(v_bind_tier_ids) loop
      update public.ticket_tiers
      set
        day_id = v_bind_day_ids[v_bind_idx],
        updated_at = now()
      where id = v_bind_tier_ids[v_bind_idx]
        and event_id = p_event_id;
    end loop;
  end if;
...
comment on function public.publish_event_v2(uuid, jsonb) is
  'Event Creator V2: unpacks draft JSON into events/venues/ticket_tiers/schedule_days. Persists ticket_type, category, sale windows, refund_policy, checkout_message, absorb_fees and access_link. Clears venue_map when the seating plan is off.';
