-- P62 Unified hybrid inventory: seated + general + add-ons + bundles in one event.
-- Holds remain orders.pending (8 min from created_at) + event_seating_units.reserved_until.
...
set tier_type = case
  when layout_type in ('numbered_seat', 'table_combo') then 'seated'
  when category = 'bundle' then 'bundle'
  else 'general'
end
where tier_type is null;

alter table public.ticket_tiers
  alter column tier_type set default 'general';

update public.ticket_tiers
set tier_type = 'general'
where tier_type is null;
...
alter table public.ticket_tiers
  drop constraint if exists ticket_tiers_tier_type_check;

alter table public.ticket_tiers
  add constraint ticket_tiers_tier_type_check
  check (tier_type in ('seated', 'general', 'addon', 'bundle'));

comment on column public.ticket_tiers.tier_type is
  'seated = mapa numerado; general = campo/capacidad; addon = extras; bundle = combo promocional.';

comment on column public.ticket_tiers.bundle_items is
  'JSON [{tier_id, quantity}] de inventario incluido en un combo. Vacío si no es bundle.';

-- -----------------------------------------------------------------------------
-- reserve_unified_cart_tx: one 8-minute pending order for mixed carts
-- (at most one seating_unit_id + general/addon/bundle lines).
-- -----------------------------------------------------------------------------
