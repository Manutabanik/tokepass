-- P162: clasificación comercial de ticket_tiers (ticketType del draft).
-- standard = entrada, combo = pack/promo, extra = adicional de upselling.

alter table public.ticket_tiers
  add column if not exists ticket_type text not null default 'standard';
...
alter table public.ticket_tiers
  ...
  check (ticket_type in ('standard', 'combo', 'extra'));

comment on column public.ticket_tiers.ticket_type is
  'Clasificación de checkout: standard | combo | extra. Alias de producto: ticketType.';

update public.ticket_tiers
set ticket_type = 'extra'
...
update public.ticket_tiers
set ticket_type = 'combo'
where ticket_type = 'standard'
  and (
    tier_type = 'bundle'
    or category = 'bundle'
    or bundle_type is not null
  );
...
comment on function public.publish_event_v2(uuid, jsonb) is
  'Event Creator V2: unpacks draft JSON into events/venues/ticket_tiers. Persiste ticket_type (standard|combo|extra).';
